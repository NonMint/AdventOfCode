...
var addsTo2020 = (a, b, c = 0) => {
	return (a + b + c) === 2020;
}

var day1Answer1 = (report) => {
	var item = report.shift();
	
	console.log(`Run ${200 - report.length}, item: ${item}`);

	for (var i = 0; i < report.length; i++)
	{
		if (addsTo2020(report[i], item)) {
			console.log(`Found our solution on run ${200 - report.length}, solution is ${item * report[i]}`);
			return;
		}
	}
...
day1Answer1([...expenseReport]);

var day1Answer2 = (report) => {
  var item = report.shift();
  ...
  for (var i = 0; i <= report.length; i++)
  {
    ...
    {
      if (addsTo2020(item, report[i], report[j]))
      {
        console.log(`Found our solution on run ${200 - report.length}, solution is ${item * report[i] * report[j]}`);
        return;
      }
    }
  }
